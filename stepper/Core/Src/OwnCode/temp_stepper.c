...
void set_speed(StepperContext* StepperContext, int steps_per_second);
...
//---------Basic---Functions--------------
static void* StepLibraryMalloc(unsigned int size) { return malloc(size); }

static void StepLibraryFree(const void* const ptr) { free((void*)ptr); }
...
static void StepLibraryDelay() { return; }
...
static int StepTimerCancelAsync(void* pPWM) {
    ...
    if (StepperContext.is_running) {
        HAL_TIM_OnePulse_Stop_IT(StepperContext.htim1_handle, TIM_CHANNEL_1);
...
static int reset(StepperContext* StepperContext) {
...
static int Position( StepperContext* StepperContext, int argc, char** argv) {
            int position;
        L6474_GetAbsolutePosition(StepperContext->h, &position);
        printf("Current position: %d\n\r", (position * MM_PER_TURN) / (STEPS_PER_TURN * RESOLUTION));
}

static int Status(StepperContext* StepperContext, int argc, char** argv) {
...
static int move(StepperContext* StepperContext, int argc, char** argv) {
...
static int config(StepperContext* StepperContext, int argc, char** argv) {
    if (argc < 2) {
        printf("Invalid number of arguments\r\n");
        return -1;
    }
...
//-----helper-functions---------
static int powerena(StepperContext* StepperContext, int argc, char** argv) {
    if (argc == 2) {
...
void set_speed(StepperContext* StepperContext, int steps_per_second) {
    ...
    __HAL_TIM_SET_PRESCALER(StepperContext->htim4_handle, i);
    __HAL_TIM_SET_AUTORELOAD(StepperContext->htim4_handle, (quotient / (i + 1)) - 1);
    StepperContext->htim4_handle->Instance->CCR4 = StepperContext->htim4_handle->Instance->ARR / 2;
}

static int Initialize(StepperContext* StepperContext) {
...
static int StepperHandler(int argc, char** argv, void* ctx) {
    StepperContext* StepperContext = (StepperContext*)ctx;
    int result = 0;
...
void start_tim1(int pulses) {
    ...
    if (current_pulses != 1) {
        HAL_TIM_OnePulse_Stop_IT(StepperContext.htim1_handle, TIM_CHANNEL_1);
        __HAL_TIM_SET_AUTORELOAD(StepperContext.htim1_handle, current_pulses);
        HAL_TIM_GenerateEvent(StepperContext.htim1_handle, TIM_EVENTSOURCE_UPDATE);
        HAL_TIM_OnePulse_Start_IT(StepperContext.htim1_handle, TIM_CHANNEL_1);
        __HAL_TIM_ENABLE(StepperContext.htim1_handle);
    } else {
        StepperContext.done_callback(StepperContext.h);
    }
}
...
void InitStepper(ConsoleHandle_t console_handle, SPI_HandleTypeDef* hspi1, TIM_HandleTypeDef* tim1_handle, TIM_HandleTypeDef* tim4_handle) {
    HAL_GPIO_WritePin(STEP_SPI_CS_GPIO_Port, STEP_SPI_CS_Pin, 1);
    ...
    StepperContext.h = L6474_CreateInstance(&p, hspi1, NULL, tim1_handle);
    StepperContext.htim1_handle = tim1_handle;
    StepperContext.htim4_handle = tim4_handle;
    StepperContext.state = scs.INIT;
    StepperContext.error_code = 0;

    CONSOLE_RegisterCommand(console_handle, "stepper", "Stepper main Command", StepperHandler, &StepperContext);
}