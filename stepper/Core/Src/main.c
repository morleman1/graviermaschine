...
/* USER CODE BEGIN PFP */
void SpindleInit(ConsoleHandle_t consoleHandle, TIM_HandleTypeDef tim1Handle);
void StepperInit(ConsoleHandle_t consoleHandle, TIM_HandleTypeDef tim1Handle, TIM_HandleTypeDef tim4Handle, SPI_HandleTypeDef hspi1Handle);
void InitTask();
...
void InitTask()
{
  ...
  ConsoleHandle_t consoleHandle = CONSOLE_CreateInstance(4 * configMINIMAL_STACK_SIZE, configMAX_PRIORITIES - 5);
  CONSOLE_RegisterCommand(consoleHandle, "capability", "Shows what the program is capable of", CapabilityFunc, NULL);
 
  SpindleInit(consoleHandle, htim2);
  StepperInit(consoleHandle, htim1, htim4, hspi1);
